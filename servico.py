...
def listar_servicos_por_ano(dia_inicio: int,  mes_inicio: int, ano_inicio: int, dia_final: int, mes_final: int, ano_final: int, session: Session = Depends(get_session)):
    statement = select(Servico).where(Servico.cadastrado_em.between(f"{ano_inicio}-{mes_inicio}-{dia_inicio}", f"{ano_final}-{mes_final}-{dia_final}"))
    # Consulta
...
# Consulta por especialidade e peca
@router.get("/servicos/consulta_esp_peca/", response_model= list[Servico])
def servicos_por_especialidade_e_peca(especialidade: str, peca_id: int, db: Session = Depends(get_session)):
    consult = (
        select(Servico).options(joinedload(Servico.tecnico),joinedload(Servico.pecas)).where(Tecnico.especialidade == especialidade).where(ServicoPeca.peca_id == peca_id)
    )
    result = db.exec(consult).all()
    return result
